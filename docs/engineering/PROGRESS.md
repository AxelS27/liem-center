...
- 2026-06-08 — Added pin-to-profile (pin icon, pinned products sort to the top of the library and drive the profile showcase via a shared localStorage store) and a working cart (Add to cart on product detail, cart-driven checkout, nav cart-count badge) via `useCart`/`usePinnedProducts` hooks. Build, lint, typecheck green.
- 2026-06-08 — Built app surfaces on mock data: `/library` (source filter, ownership + invite status pills, retry/connect/download actions), `/orders` + `/orders/[id]`, `/profile` (header, stats, tier progress, pinned showcase, badges, activity), `/checkout` (cart, coupon, Midtrans pay stub), `/wishlist`, `/notifications` (mark read), `/settings` (Profile/Connected accounts/Notifications/Security tabs). Added shared `StatusPill` + `--success`/`--warning` tokens. `pnpm --filter @repo/web build` compiles every route; lint + typecheck green.
